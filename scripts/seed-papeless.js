...
async function importProduits() {
  for (const produit of produits) {
    ...
    let imagesSecondaire = null;

    if (produit.imagePrincipale) {
      imagePrincipale = await checkFileExistsBeforeUpload([
        produit.imagePrincipale,
      ]);
    }
    if (produit.imagesSecondaire && produit.imagesSecondaire.length > 0) {
      imagesSecondaire = await checkFileExistsBeforeUpload(
        produit.imagesSecondaire
      );
    }
...
async function importSeedData() {
  await setPublicPermissions({
    ...
    commande: ["find", "findOne"],
  });

  await importCategories();
  await importProduits();
  await importCommandes();
}
